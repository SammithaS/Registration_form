...
return(
    ...
    className={index!==newStep.length -1? "w-full flex items-center":"flex items-center"}>
       ...
       <div className='relative flex flex-col items-center text-blue-600'>
              ...
              <div className={`absolute top-0 text-center px-7 mt-16 w-32 text-xs font-medium uppercase ${step.highlighted? "text-gray-900":"text-gray-400"}`}>
                 {/* display description */}
                 {step.description}
              </div>
          </div>
          <div className={`flex-auto border-t-2 transition duration-500 ease-in-out ${step.completed?"border-blue-500":"border-gray-300"}`}> 
          {/*Display line */} 
          </div>
    </div>
);
